...
public class CasesController : ControllerBase
{
    ...
    public async Task<IActionResult> Create([FromBody] CreateCaseDto dto, CancellationToken cancellationToken)
    {
    ...
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    public async Task<IActionResult> Update(Guid id, [FromBody] UpdateCaseDto dto, CancellationToken cancellationToken)
    {
        var result = await _service.UpdateAsync(id, dto, cancellationToken);
        if (!result.Success)
            return result.Message == "Case not found." ? NotFound(result) : BadRequest(result);
        return Ok(result);
    }
