...
public static class CorporateBulkUploadFileParser
{
    public sealed class RawRow
    {
        ...
        public string IncorporatedCountry { get; init; } = string.Empty;
        public string DateOfIncorporation { get; init; } = string.Empty;
        public string CompanyReferenceCode { get; init; } = string.Empty;
        public string TradeLicense { get; init; } = string.Empty;
    }
    ...
    private sealed class ColumnMap
    {
        ...
        public int IncorporatedCountry = -1;
        public int DateOfIncorporation = -1;
        public int CompanyRef = -1;
        public int TradeLicense = -1;
    }
    ...
    private static ColumnMap MapHeaderColumns(IReadOnlyList<string> headerCells)
    {
        ...
        for (var i = 0; i < headerCells.Count; i++)
        {
            ...
            if (h.Contains("customer id", StringComparison.OrdinalIgnoreCase) && map.CustomerId < 0)
                map.CustomerId = i;
            else if ((h.Contains("full name", StringComparison.OrdinalIgnoreCase) && h.Contains("entity", StringComparison.OrdinalIgnoreCase)) && map.FullName < 0)
                map.FullName = i;
            else if (h.Contains("incorporated country", StringComparison.OrdinalIgnoreCase) && map.IncorporatedCountry < 0)
                map.IncorporatedCountry = i;
            else if ((h.Contains("date of incorporation", StringComparison.OrdinalIgnoreCase) || h.Contains("incorporation", StringComparison.OrdinalIgnoreCase) && h.Contains("date", StringComparison.OrdinalIgnoreCase)) && map.DateOfIncorporation < 0)
                map.DateOfIncorporation = i;
            else if (h.Contains("company reference", StringComparison.OrdinalIgnoreCase) && map.CompanyRef < 0)
                map.CompanyRef = i;
            else if (h.Contains("trade license", StringComparison.OrdinalIgnoreCase) && map.TradeLicense < 0)
                map.TradeLicense = i;
        }
        ...
        if (map.FullName < 0)
        {
            ...
            {
                var h = NormalizeHeader(headerCells[i]);
                if (h.Contains("full name", StringComparison.OrdinalIgnoreCase) && map.FullName < 0)
                    map.FullName = i;
            }
        }

        if (map.CustomerId < 0 || map.FullName < 0)
            throw new InvalidOperationException("Required columns not found: Customer ID and Full Name of the Entity.");
    ...
    private static RawRow BuildRow(int lineIndex, IReadOnlyList<string> cells, ColumnMap col)
    {
        ...
        return new RawRow
        {
            ...
            IncorporatedCountry = G(col.IncorporatedCountry),
            DateOfIncorporation = G(col.DateOfIncorporation),
            CompanyReferenceCode = G(col.CompanyRef),
            TradeLicense = G(col.TradeLicense)
        };
    }
    ...
    private static int FindHeaderRow(ISheet sheet)
    {
        for (var r = 0; r <= Math.Min(10, sheet.LastRowNum); r++)
        {
            ...
            if (joined.Contains("customer") && (joined.Contains("entity") || joined.Contains("full name")))
                return r;
        }
