...
const TOKEN_KEY = 'aml_admin_access_token';
const REFRESH_KEY = 'aml_admin_refresh_token';
const EXPIRES_KEY = 'aml_admin_expires_at';
...
export class AuthService {
  ...
  login(credentials: LoginRequest): Observable<ApiResponse<LoginResponse>> {
    ...
    return this.http.post<ApiResponse<LoginResponse>>(`${this.apiUrl}/api/Auth/login`, body).pipe(
      tap((res) => {
  ...
  refresh(): Observable<ApiResponse<LoginResponse> | null> {
    ...
    return this.http
      .post<ApiResponse<LoginResponse>>(`${this.apiUrl}/api/Auth/refresh`, { refreshToken: refresh })
      .pipe(
        tap((res) => {
  ...
  logout(): void {
    ...
    if (refresh) {
      this.http
        .post(`${this.apiUrl}/api/Auth/revoke`, { refreshToken: refresh }, { headers: this.authHeaders() })
        .subscribe({ error: () => {} });
    }
    this.clearTokens();
