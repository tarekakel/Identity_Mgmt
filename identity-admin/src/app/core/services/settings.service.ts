...
const STORAGE_KEY = 'identity_admin_settings';
...
function migrateFromOldFormat(parsed: Record<string, unknown>): Partial<UserSettings> {
  ...
  const oldFontSize = parsed['fontSize'] as string | undefined;
  if (oldFontSize === 'small') result.fontSizePx = 14;
  else if (oldFontSize === 'medium') result.fontSizePx = 16;
  else if (oldFontSize === 'large') result.fontSizePx = 18;
  return result;
}
